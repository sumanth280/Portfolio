...
(function () {
   ...
   */
  window.addEventListener('load', () => {
    // There are multiple elements with class `portfolio-container` in the page.
    // Ensure we initialize Isotope on the one that actually contains the items
    // and use a left-aligned row layout for proper alignment after filtering.
    let portfolioContainers = select('.portfolio-container', true);
    let portfolioContainer = portfolioContainers && portfolioContainers.length
      ? portfolioContainers[portfolioContainers.length - 1]
      : null;
